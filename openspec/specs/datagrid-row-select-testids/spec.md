# datagrid-row-select-testids Specification

## Purpose
Specifies the `data-testid` attributes emitted on the MUI DataGrid's built-in selection checkboxes (per-row `__check__` column cells and the header select-all). Generated tables route MUI's `baseCheckbox` slot through a `CustomCheckbox` wrapper that derives stable testids from the table's XMI id and the row's `__identifier`, enabling Playwright `getByTestId(...)` locators in place of label-based fallbacks.
## Requirements
