# filter-dialog-chrome-and-operators Specification

## Purpose
Specifies the `data-testid` attributes emitted by the per-Filter `FilterDialog` component: the Add-new-filter `DropdownButton` (`${filterXmiId}-add-new-filter`) and each operator `MenuItem` (`filter-operator-<kebab-cased-operator>`, e.g. `filter-operator-not-equal`). Ensures unique testids per operator option so Playwright strict-mode locators can address them individually.
## Requirements
