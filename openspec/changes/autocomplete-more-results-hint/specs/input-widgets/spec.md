...
The footer SHALL be visible whether the search input is empty or non-empty. Its gates are `options.length >= limit` **and** `loading === false`. The footer SHALL be hidden when the returned options count is below the limit (i.e. the server is known to have returned every match), and SHALL be hidden while a query is in flight — the footer describes a **completed** result set, so it must not assert a count over options that are being replaced.
...
#### Scenario: Footer hidden while a query is in flight

- **GIVEN** a widget with `autoCompleteLimit={10}` whose previous search returned a full page of 10 options, so the footer is visible
- **WHEN** the user edits the search text and the debounced `handleSearch` sets `loading` to true
- **THEN** the footer is hidden for the duration of the request, rather than continuing to assert "showing the first 10" over the outgoing result set
- **AND** it reappears only if the completed response again contains at least `limit` options

#### Scenario: Footer fill follows the dropdown's rounded corners

- **GIVEN** a theme whose `MuiPaper.rounded` sets a non-zero `borderRadius` (the generated default is `density.borderRadius`, 20)
- **WHEN** the dropdown is open with the footer visible as the Paper's last child
- **THEN** the footer's filled background is clipped to the Paper's bottom corner radii and does not extend past the curve
