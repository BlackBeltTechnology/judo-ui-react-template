...
- **Integration tests** (`judo-ui-react-itest/**`): mechanical snapshot refresh for every committed widget-level snapshot that renders one of the four widgets, plus every `system_*.json` snapshot. Same procedure as JNG-6409 §7.2. The `judo-diff-checker-maven-plugin` will list every drift on the first CI run.
