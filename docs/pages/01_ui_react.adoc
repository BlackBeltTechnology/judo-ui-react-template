...
For template overrides the generated source can contain more detailed information, for example what was
the URI of the template, what was the included fragment file etc. To achive that
have to add `<debugPrint>true</debugPring>` into `templateVariables` section of `pom.xml`
