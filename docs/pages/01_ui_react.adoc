...
For details about Pandino, please check it's corresponding documentation.
...
`INTERFACE_KEY` s for pages can be found at `src/routes.tsx` with their actual implementation pairs next to them.
...
The localization loader is responsible to load the translations for the application.
