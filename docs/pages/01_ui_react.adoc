...
The pattern with regard to how can this be achieved is similar to the previous.
...
const galaxiesCreateFormErrorHook: ErrorProcessorHook<ViewGalaxy> = () => {
  ...
  return (error: any, defaultResults: ErrorProcessResult, options?: ErrorHandlingOption, payload?: ViewGalaxy) => {
    ...
    if (error?.response?.status === 400) {
      ...
      // if the host page has set the `setValidation` option
      if (typeof options?.setValidation === 'function') {
        // filter errors where we know what field is affected
    ...
    // fail-safe mode of making sure we do not error out
...
The best way to find out what services require what parameters, you only need to search for the `useErrorHandler` hook's
