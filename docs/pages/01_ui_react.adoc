...
This hook cannot fetch all info when used inside dialogs. It is recommended to use useViewContext() instead.
...
In order to find out the XMIID of a page/dialog, we can use the inspector in the browser and check the `data-testid`
attribute of the page/dialog. For the generic parameter we can search for the corresponding ViewModel typedef in the
src folder based on the XMIID.
...
const PageXMIID = 'GenericUser/(esm/_ZS1VYIgBEfC3naMpyl8-bQ)/RelationFeatureView';
const { refresh } = getForPage<ServicesFault_registry_servicesRegistryHeaderRegistryHeader_View_EditViewModel>(PageXMIID);
----
...
Of course we can re-implement this page as we see fit, we just need to copy the `src/custom/hooks/default-guest-page-hook-registration.tsx.default`
file and remove the ".default" suffix. Once this is done, we can use the registration function to register our implementation.
